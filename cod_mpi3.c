...
int main(int argc, char *argv[]) {
    ...
    if (size != 4) {
        if (rank == 0) printf("Este programa requer exatamente 4 processos (1 master + 3 ilhas).\n");
        MPI_Finalize();
        return 1;
    }
            ...
            if (geracao % INTERVALO_MIGRACAO == 0) {
                ...
                for (int i = 0; i < NUM_ILHAS - 1; i++) {
                    for (int j = i + 1; j < NUM_ILHAS; j++) {
                        if (melhores[indices[j]].fitness > melhores[indices[i]].fitness) {
                            int tmp = indices[i];
                            indices[i] = indices[j];
                            indices[j] = tmp;
                        }
                ...
                migrantes[0] = melhores[indices[2]]; // 3º melhor vai para ilha 0
                migrantes[1] = melhores[indices[0]]; // 1º melhor vai para ilha 1
                migrantes[2] = melhores[indices[1]]; // 2º melhor vai para ilha 2
                // Envia migrantes
                for (int ilha = 0; ilha < NUM_ILHAS; ilha++) {
